...
set search_path = ''
as $$
  select coalesce(p.workspace_owner_id, p.id, (select auth.uid()))
...
as $$
  ...
  scored as (
    ...
    where lc.user_id = public.current_workspace_owner_id()
      and li.user_id = public.current_workspace_owner_id()
...
revoke execute on function public.current_workspace_owner_id() from public,anon;
