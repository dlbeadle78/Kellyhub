...
import {BookOpen,CalendarPlus,CheckCircle2,Clock,Target} from 'lucide-react'
import {supabase} from './supabase.js'
import {LEARNING_CONTENT,LEARNING_STATUS} from './learningContent.js'
import './phase2-progress.css'
...
export default function Phase2Progress({session,subjects=[],tasks=[],practice=[],skills=[],timetable=[],events=[],notify,go}){
  ...
  return <div className="p2p-shell">
    ...
    <section className="p2p-board">{columns.map(([status,label])=><div key={status}><h3>{label}</h3>{tasks.filter(t=>t.status===status).slice(0,8).map(t=><article key={t.id}><strong>{t.title}</strong><span>{subjectNames[t.subject_slug]||'General'}</span><small>{t.next_action||'Open task for next step'}</small></article>)}{!tasks.some(t=>t.status===status)&&<p className="p2p-empty">Nothing here.</p>}</div>)}<div className="finished"><h3><CheckCircle2/> Finished this week</h3>{finished.slice(0,8).map(t=><article key={t.id}><strong>{t.title}</strong><span>{subjectNames[t.subject_slug]||'General'}</span></article>)}{!finished.length&&<p className="p2p-empty">Finished work will stay visible here.</p>}</div></section>
    <div className="p2p-grid"><section className="p2p-card"><div className="p2p-title"><Target/><div><strong>What should I revise?</strong><span>Specific topics based on learning confidence</span></div></div>{recommended.map((topic,index)=><article className="p2p-revision" key={`${topic.unitSlug}:${topic.topicSlug}`}><div className="p2p-rank">{index+1}</div><div><strong>{subjectNames[topic.subjectSlug]} · {topic.title}</strong><span>{LEARNING_STATUS[topic.status]||topic.status}</span><p>{topic.status==='needs_review'?'Revisit the detailed explanation and evidence, then retry recall without revealing answers.':topic.status==='developing'?'Use retrieval plus one short application/evaluation activity.':'Start with the core explanation, then move into A-level depth and recall.'}</p><small><Clock/> About {Math.max(20,Math.min(45,topic.time||25))} minutes</small></div><div className="p2p-plan"><select id={`slot-${index}`} defaultValue="0">{slots.length?slots.map((s,i)=><option value={i} key={s.start.toISOString()}>{s.label}</option>):<option value="evening">Tomorrow 18:00</option>}</select><button disabled={busy===topic.topicSlug} onClick={()=>{const select=document.getElementById(`slot-${index}`);const slot=select?.value==='evening'?null:slots[Number(select?.value||0)];plan(topic,slot)}}><CalendarPlus/> Plan it</button></div></article>)}</section>
      <aside className="p2p-side"><section className="p2p-card"><div className="p2p-title"><BookOpen/><div><strong>Practice record</strong><span>{practice.length} activities recorded</span></div></div>{practice.slice(0,5).map(p=><div className="p2p-simple" key={p.id}><span>{subjectNames[p.subject_slug]||'General'} · {p.title}</span><strong>{p.score!=null?`${p.score}${p.score_out_of?`/${p.score_out_of}`:''}`:'Done'}</strong></div>)}<button onClick={()=>go?.('practice')}>Open Mock & Practice</button></section><section className="p2p-card"><strong>Planned revision</strong>{recs.filter(r=>r.status==='planned').slice(0,5).map(r=><div className="p2p-planned" key={r.id}><span>{r.title}</span><small>{r.planned_for?fmt(r.planned_for):'Planned'}</small></div>)}{!recs.some(r=>r.status==='planned')&&<p className="p2p-empty">Nothing planned from recommendations yet.</p>}</section><section className="p2p-card"><strong>Independent living</strong><p>{skills.filter(s=>s.status==='confident').length} of {skills.length} tracked skills currently marked confident.</p></section></aside></div>
  </div>
}
